<script lang="ts">
	...
	$: previewURLs = preview && files ? Array.from(files).map(URL.createObjectURL) : undefined
</script>
...
<div class="flex flex-col gap-1 {$$props.class}">
	...
	{/if}
	<label class="w-max relative cursor-pointer space-x-2 flex flex-row items-center">
		<slot name="button">
			<Button>Browse</Button>
		</slot>
		{#if previewURLs}
			<div class="flex flex-row items-center -space-x-1 hover:space-x-1 z-20 h-8">
