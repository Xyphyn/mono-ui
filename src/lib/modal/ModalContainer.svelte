...
{#if $shownModal}
	...
	>
		...
		{#if $shownModal.actions}
			<div
				class="flex items-center gap-2 {$shownModal.actions.length >= 3 ? 'flex-col' : 'flex-row'}"
			>
				{#each $shownModal.actions as action}
					<Button size="lg" class="flex-1 w-full" on:click={action.action} color={action.type}>
						{#if action.icon}
